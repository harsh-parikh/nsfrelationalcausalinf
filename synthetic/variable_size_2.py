...
# embedding sizes
class_size = np.random.normal(50, 30, max(courses) + 1).clip(10).astype("int")
class_inst = np.random.normal(2, 2, max(courses) + 1).clip(1).astype("int")
...
difficulty = np.array([random.random() < p_difficult_course
                       for _ in courses], dtype=np.float32).reshape(-1, 1)
skill = np.array([random.random() < p_skilled_instructor
                  for _ in instructors], dtype=np.float32).reshape(-1, 1)
intelligence = np.array([random.random() < p_intelligent_student
                  for _ in students], dtype=np.float32).reshape(-1, 1)

registered = []
lecturers = []

for c in courses:
    course_students = random.sample(students, class_size[c])
    registered.append([intelligence[s][0] for s in course_students])

    course_instructors = random.sample(instructors, class_inst[c])
...
for e in embeddings:
    p = logistic(e[0] + e[3] - 3 * e[1] - e[2] - 1)
    p_labels.append([p])
...
pad_grounding = torch.cat((torch.Tensor(difficulty),
                       torch.Tensor(pad_registered),
                       torch.Tensor(pad_lecturers),
                       torch.Tensor(class_size.reshape(-1, 1))), dim=1)

registered_t = [torch.Tensor(r) for r in registered]
difficulty_t = torch.Tensor(difficulty)
lecturers_t = [torch.Tensor(l) for l in lecturers]
...
for i in range(epochs):
    ...
    var_outputs = []
    for c in courses:
        var_outputs.append(var_model(difficulty_t[c], lecturers_t[c],
                                     registered_t[c]))
...
roc_auc = auc(fpr, tpr)
plt.plot(fpr, tpr, 'orange', label = 'Padded Embed Learning, AUC = %0.2f'
                                     % roc_auc)

fpr, tpr, threshold = roc_curve(labels, var_output.detach().numpy())
roc_auc = auc(fpr, tpr)
plt.plot(fpr, tpr, 'b', label = 'Variable Embed Learning, AUC = %0.2f'
                                % roc_auc)
