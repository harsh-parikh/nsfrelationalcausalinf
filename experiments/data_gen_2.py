...
def generate_data(n_auth,n_inst,n_paper,n_conf):
    ...
    coauthors = {a:{'coauthor_set':set()} for a in range(0,len(df_auth))}
    for p in range(0,len(df_paper)):
        p_auth = df_paper.loc[p]['authors']
        for at in p_auth:
            coauthors[at]['coauthor_set'] = coauthors[at]['coauthor_set'].union(set(p_auth)-set([at]))
    ...
    for p in range(0,len(df_paper)):
        ...
        relce_p = 0.5*(np.mean(ca_prestige_vec)>8.5)
